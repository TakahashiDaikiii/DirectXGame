...
#pragma endregion
    ...
    while (true) {
        ...
        // プリミティブ形状の設定コマンド
        dxCommon_->GetCommmandList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST); // 三角形リスト
        // パイプラインステートとルートシグネチャの設定コマンド
        dxCommon_->GetCommmandList()->SetPipelineState(pipelineState.Get());
        dxCommon_->GetCommmandList()->SetGraphicsRootSignature(rootSignature.Get());
        // 頂点バッファビューの設定コマンド
        dxCommon_->GetCommmandList()->IASetVertexBuffers(0, 1, &vbView);
        // インデックスバッファビューの設定コマンド
        dxCommon_->GetCommmandList()->IASetIndexBuffer(&ibView);
        // 定数バッファビュー(CBV)の設定コマンド
        dxCommon_->GetCommmandList()->SetGraphicsRootConstantBufferView(0, constBuffMaterial->GetGPUVirtualAddress());
        // SRVヒープの設定コマンド
        ID3D12DescriptorHeap* descHeaps[] = {srvHeap.Get()};
        dxCommon_->GetCommmandList()->SetDescriptorHeaps(1, descHeaps);
        ...
        dxCommon_->GetCommmandList()->SetGraphicsRootDescriptorTable(1, srvGpuHandle);

        // 全オブジェクトについて処理
        for (int i = 0; i < _countof(object3ds); i++)
        {
            DrawObject3d(&object3ds[i], dxCommon_->GetCommmandList(), vbView, ibView, _countof(indices));
        }
